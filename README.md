...
| Model discovery | Probes all Pi models | Adds `--scoped` (Pi enabled-models only) and `--free` (OpenRouter free tier only, sorted by collection popularity) flags for `init` and `update` |
